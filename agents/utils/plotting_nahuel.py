...
def plot_reward_per_episode(reward_ep, ax=None, label=None):
...
def plot_steps_per_episode(timesteps_ep, ax=None, label=None):
    ...
    if label:
        ax.plot(np.array(range(0, len(episode_steps))), episode_steps, label=label)
    else:
        ax.plot(np.array(range(0, len(episode_steps))), episode_steps)

    
    ax.set_title('Pasos (timesteps) por episodio')
    
    
def plot_steps_per_episode_smooth(timesteps_ep, ax=None, label=None):
    episode_steps = np.array(timesteps_ep)
